...
class RecruiterReportController extends Controller
{
    /**
    ...
    {
        abort_if(Gate::denies('recruiter_reports_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        if ($roleName == 'Recruiter') {
            $candidates = Candidate::select('candidate_id', 'name')->where('user_id', $user_id)->get();
        } else {

            $candidates = Candidate::select('candidate_id', 'name')->get();
        }
        ...
        $getAllData = RecruiterReportLog::orderBy('id', 'desc')
            ->select(
                'id',
                'link',
        ...
        return view('admin.recruiterReports.index', compact('jobs', 'getAllData', 'candidates', 'recruiterUsers'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $query = DB::table('applied_jobs as a')
            ...
            ->select('e.job_title', 'e.status as job_status', 'u.first_name', 'c.name', 'c.location', 's.status', 's.field_status')
            ->whereBetween('e.job_start_date', [$from_date, $to_date]);


        if ($request->has('recruiter_id') && (empty($request->recruiter_id)) && (($roleName == 'Super Admin') || ($roleName == 'Admin'))) {
            $recruiter_id = "All";
        } else if ($request->has('recruiter_id') && (!empty($request->recruiter_id)) && (($roleName == 'Super Admin') || ($roleName == 'Admin'))) {
            $recruiter_id = $request->recruiter_id;
        ...
        if ($request->has('candidate_id') && (empty($request->candidate_id))) {
            $candidate_id = "All";
        } else if ($request->has('candidate_id') && (!empty($request->candidate_id))) {
            $candidate_id = $request->candidate_id;
        ...
        $excel_path =  $this->recruiterGenerateReportXlsx($data);
        ...
        $inserReport = RecruiterReportLog::create($inserdata);
        $query1 = RecruiterReportLog::orderBy('id', 'desc');
        if (in_array($roleName, ['Super Admin', 'Admin'])) {
            $query1->select(
                'id',
                'link',
    ...
    public function recruiterGenerateReportXlsx($data)
    {
        ...
        if (!file_exists(public_path('/recruiter_report_generate/'))) {
            mkdir(public_path('/recruiter_report_generate/'));
        }

        $writer->save(public_path('/recruiter_report_generate/' . $filename));
        return '/recruiter_report_generate/' . $filename;
    }
}
