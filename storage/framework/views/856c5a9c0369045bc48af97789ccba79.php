<script>
    ...
    $(document).ready(function() {
        ...
        $(".add_contract").on("click", function() {
            ...
            newContractSection.attr("id", "contract_section_" + sectionCount);
            ...
            newContractSection.find("input, select, div").each(function() {
                ...
                if (currentName) {
                    var newName = currentName.replace(/\[\d+\]$/, '[' + sectionCount + ']');
                ...
                if ($(this).is(':checkbox')) {
                    ...
                    // Add the value attribute if not present
                    if (!$(this).attr('value')) {
                        $(this).attr('value', sectionCount - 1);
                    }
                }
            ...
            $('#checklist_upload_row_' + sectionCount).css('display', 'none');
    ...
    $('#employerEditFormId').validate({
        ...
        rules: {
            ...
            email: {
                ...
                email: true,
                remote: {
                    url: '<?php echo e(route('admin.check-email-unique-employer')); ?>',
                    type: 'post',
                    data: {
                        userId: function() {
                            return $('#userId').val();
    ...
    function initializeValidation() {
        ...
        $("[name^=contract_type]").each(function(index) {
        ...
        $("[name^=checklist_upload]").each(function(index) {

            var currentIndex = index + 1;

            if ($(`[name^='assign_contract_id[${currentIndex}]']`).val() =='') {

            $(this).rules("remove", "required");
            $(this).rules("add", {
                required: {
                    depends: function(element) {
                        return $("#contract_type_" + currentIndex).val() == 1;
                    },
                },
                extension: "pdf|doc|docx",
        ...
        $("[name^=start_date]").each(function(index) {
            var currentIndex = index + 1;
            $(this).rules("remove", "required");
            $(this).rules("add", {
                required: true,
            ...
            $('#input_start_date_' + currentIndex).datepicker({
                templates: {
            ...
            $('#input_end_date_' + currentIndex).datepicker({
                templates: {
                    leftArrow: '<i class="fa fa-chevron-left"></i>',
                    rightArrow: '<i class="fa fa-chevron-right"></i>'
                },
            ...
            $('#input_start_date_' + currentIndex).datepicker().on("changeDate", function() {

                var startDate = $('#input_start_date_' + currentIndex).datepicker('getDate');
                ...
                $('#input_end_date_' + currentIndex).datepicker('setStartDate', oneDayFromStartDate);

                // Clear the selected date in input_end_date
                $('#input_end_date_' + currentIndex).datepicker('clearDates');
            });

            $('#input_end_date_' + currentIndex).datepicker().on("show", function() {
        ...
        $("[name^=contract_upload]").each(function(index) {

            var currentIndex = index + 1;

            if ($(`[name^='assign_contract_id[${currentIndex}]']`).val() =='') {
                $(this).rules("remove", "required");
                $(this).rules("remove", "extension");
                $(this).rules("add", {
                    required: true,
                    extension: "pdf|doc|docx",
                });
...
<?php /**PATH /var/www/html/laravel/bod/resources/views/admin/customJs/employer/edit.blade.php ENDPATH**/ ?>