...
<div class="card">
    ...
    <div class="card-header">
        {{ trans('global.applyjob') }} as {{ $getJobDetail->job_title }}
    </div>
    ...
    <div class="card-header">
        ...
        {{ trans('cruds.appliedJobs.title_singular') }} {{ trans('cruds.candidates.title_singular') }} {{ trans('global.list') }}
    </div>
        ...
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-User">
                ...
                <tbody>
                    ...
                    @foreach($appliedJobCandidates as $key => $candidateJobActiveStatus)
                        {{-- {{dd($candidateJobActiveStatus->job_id)}} --}}
                    <tr data-entry-id="{{ $candidateJobActiveStatus->id }}">
                        <td>
                            {{ ($key + 1) ?? '' }}
                        </td>
                        <td>
                            {{ $candidateJobActiveStatus->candidate->user_id ?? '' }}
                        </td>
                        <td>
                            {{ $candidateJobActiveStatus->candidate->name ?? '' }}
                        </td>
                        @php
                        ...
                        <td>
                            ...
                            @if($candidateJobActiveStatus->AppliedJobActivestatus->status == 'None')
                            {{ 'Profile Recived' }}
                            @else
                            {{ $candidateJobActiveStatus->AppliedJobActivestatus->status  ?? ''  }}
                        ...
                        <td>
                            ...
                            {{ $candidateJobActiveStatus->AppliedJobActivestatus->field_status  ?? ''  }}
                        ...
                        <td>
                            ...
                            @can('candidate_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('admin.candidate.show', encrypt_data($candidateJobActiveStatus->candidate->id)) }}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            ...
                            {{-- @can('job_edit')
                            <a class="btn btn-xs btn-info" href="{{ route('admin.appliedJobs.edit',encrypt_data($candidateJobActiveStatus->AppliedJobActivestatus->id)) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                            @endcan --}}

                            @can('job_applied_delete')
                            <button class="btn btn-xs btn-danger delete-applied-btn" data-id="{{$id}}">
                                {{ trans('global.delete') }}
                            </button>
                            @endcan
                            ...
                            @php
                                ...
                                $link =getAssessmentLink($candidateJobActiveStatus->candidate_id,$candidateJobActiveStatus->AppliedJobActivestatus->status);
                            ...
                            @can('candidate_show')
                            <a class="btn btn-xs btn-success" href="{{ route('admin.appliedJobs.candidatesViewJobStatus', [encrypt_data($candidateJobActiveStatus->id), 'path' => 'admin/appliedJobs/' . $candidateJobActiveStatus->job_id]) }}">
                                {{ trans('global.status_view') }}
                            </a>                            
                            
                            @endcan

                            @if (!empty($candidateJobActiveStatus->savedCandidate))

                            <a class="btn btn-xs btn-warning mt-1" href="{{ route('admin.candidate.unSavedCandidate', ['id' =>encrypt_data($candidateJobActiveStatus->candidate_id) ]) }}">
                                un-save
                            </a>
                            @else
                            <a class="btn btn-xs btn-success" href="{{ route('admin.appliedJobs.savedCandidate', ['id' =>encrypt_data($candidateJobActiveStatus->id) ]) }}">
                                Save candidate
                            </a>
                            @endif
