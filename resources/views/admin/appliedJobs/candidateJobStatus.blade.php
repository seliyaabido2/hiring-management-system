        ...
        <div class="row bg-green mx-0">
            <div class="col-6">
                ...
                <div class="p-2 text-center text-white text-lg bg-green rounded-top"><span class="text-uppercase">Candidate Name -
                    </span><span class="text-medium candidateName">{{ $appliedJobData->candidate->name }}</span>
                </div>
            </div>
            <div class="col-6">
                ...
                <div class="p-2 text-center text-white text-lg bg-green rounded-top">
                    ...
                    <span class="text-medium">{{ $appliedJobData->candidate->email }}</span>
                </div>
            </div>
        </div>
                    ...
                    @foreach ($candidateJobStatusComment as $key => $statusitem)
                        ...
                        @if($statusitem->status =='None')
                            <li class="tab_inactive">
                                <h6><b>Profile Received</b> <button class="btn btn-xs btn-success" onclick="window.open('{{ url('admin/candidate/'.encrypt_data($appliedJobData->candidate->id)) }}');">View Data</button></h6>
                            ...
                            @if(!in_array($statusitem->field_status,['None','Stand By','No Response']))
                                ...
                                <li class="{{ $fieldclass }}">
                                    <h6><b>{{ ($statusitem->status == 'Final Selection' && $statusitem->field_status == 'Selected') ? 'Selected' : $statusitem->status }}</b>
                                        ...
                                        @if(in_array($getUserRole,['Employer','Admin','Super Admin']) && $statusitem->job_creator_id == $userId && $appliedJobData->getJobDetail->status == 'Active')
                                        ...
                                        @if(in_array($statusitem->status,['Assessment','Phone Interview','In person Interview']))
                                            ...
                                            @if(empty($inviteeCreatedPayload) && ($employer_calendly == true) && !empty($employer_schedule_url))
                                                    @if ($statusitem->status == 'Assessment')
                                                        <a href="" onclick="Calendly.initPopupWidget({url: '{{ $employer_schedule_url }}/assessment?name={{ $appliedJobData->candidate->name }}&email={{ $appliedJobData->candidate->email }}',utm: {
                                                    ...
                                                    <a href="" onclick="Calendly.initPopupWidget({url: '{{ $employer_schedule_url }}/phone-interview?name={{ $appliedJobData->candidate->name }}&email={{ $appliedJobData->candidate->email }}',utm: {
                                                        utmCampaign: {{ $appliedJobData->job_id }},
                                                        utmSource: {{ $appliedJobData->candidate_id }},
                                                    ...
                                                    <a href="" onclick="Calendly.initPopupWidget({url: '{{ $employer_schedule_url }}/in-person-interview?name={{ $appliedJobData->candidate->name }}&email={{ $appliedJobData->candidate->email }}',utm: {
                                                        utmCampaign: {{ $appliedJobData->job_id }},
                                                        utmSource: {{ $appliedJobData->candidate_id }},
                                            ...
                                            @if(!in_array($statusitem->field_status,['Stand By','No Response']))
                                                @if(in_array($getUserRole,['Employer','Admin','Super Admin']) && $statusitem->job_creator_id == $userId && $appliedJobData->getJobDetail->status == 'Active')
                                                    ...
                                                    @if ($statusitem->status != 'Background Check')
                                                         <button class="btn btn-xs btn-warning skip-status-update" onclick="SkipFieldStatus('{{$appliedJobData->candidate->name}}','{{$statusitem->candidate_id}}','{{$statusitem->job_id}}','{{$sItem}}','Skip');">skip Round</button>
...
@include('admin.customJs.appliedJobs.candidateJobStatus')
@endpush
