...
<div class="card">
    <div class="card-header">
      {{ trans('cruds.mySavedJobTeamplates.title_singular') }} {{ trans('global.list') }}
            ...
            <table class=" table table-bordered table-striped table-hover datatable datatable-User">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ trans('cruds.employerJobs.fields.job_vacancy') }}
                        </th>
                        <th>
                ...
                <tbody>
                     ...
                     @foreach($EmployerJob as $key => $job)
                        <tr data-entry-id="{{ $job->employerJob->id }}">

                            <td>
                              {{ ($key + 1) ?? '' }}
                            </td>
                            <td>
                                {{ $job->employerJob->job_title ?? '' }}
                            </td>
                            <td>
                                {{ $job->employerJob->number_of_vacancies ?? '' }}
                            </td>
                            <td>
                                {{ $job->employerJob->location ?? '' }}
                            </td>
                            <td>
                                {{ $job->employerJob->job_recruiment_duration ?? '' }} Days
                            </td>
                            <td>
                                {{ date("d-m-Y", strtotime($job->employerJob->job_start_date)) ?? '' }}
                            </td>
                            @php
                            $enddate=calculateEndDate($job->employerJob->job_start_date,$job->employerJob->job_recruiment_duration);
                            ...
                            <td>
                                ...
                                $addedby = getUserName($job->employerJob->user_id);
                            ...
                            <td>
                                ...
                                @can('job_show')
                                <a class="btn btn-xs btn-primary mt-11" href="{{ route('admin.employerJobs.show', encrypt_data($job->employerJob->id)) }}">
                                    {{ trans('global.view') }}
                                </a>
                                @endcan
                                @if(!empty($job->employerJob->savedJobTemplate))
                                <a class="btn btn-xs btn-warning mt-11" href="{{ route('admin.appliedJobs.unSavedJobTemplate', ['id' =>encrypt_data($job->employerJob->id) ]) }}">
                                    un-save Job template
                                </a>
                                @else
                                <a class="btn btn-xs btn-success mt-11" href="{{ route('admin.appliedJobs.savedJobTemplate', ['id' =>encrypt_data($job->employerJob->id) ]) }}">
                                    Save Job template
                                </a>
                                @endif
