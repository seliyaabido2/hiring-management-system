        ...
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-User">
                ...
                <tbody>
                    ...
                    {{-- {{dd($candidate)}} --}}
                    <tr data-entry-id="{{ $candidate->bod_candidate->id }}">
                        <td>
                            {{ ($key + 1) ?? '' }}
                        </td>
                        <td>
                            {{ $candidate->bod_candidate->user_id ?? '' }}
                        </td>
                        <td>
                            {{ $candidate->bod_candidate->name ?? '' }}
                        </td>
                        @php
                        ...
                        <td>
                            @if($candidate->AppliedJobActivestatus->status == 'None')
                            {{ 'Profile Recived' }}
                            @else
                            {{ $candidate->AppliedJobActivestatus->status  ?? ''  }}
                            @endif
                        </td>

                        <td>
                            @can('candidate_show')
                            <a class="btn btn-xs btn-primary" href="{{ route('admin.bodCandidate.show', encrypt_data($candidate->bod_candidate->id)) }}">
                                {{ trans('global.view') }}
                            </a>
                            @endcan
                            ...
                            {{-- @can('job_edit')
                            <a class="btn btn-xs btn-info" href="{{ route('admin.bodAppliedJobs.edit',encrypt_data($candidate->AppliedJobActivestatus->id)) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                            @endcan --}}

                            @can('job_applied_delete')
                            <button class="btn btn-xs btn-danger delete-applied-btn" data-id="{{$candidate->id}}">
                                {{ trans('global.delete') }}
                            </button>
                            @endcan
                            ...
                            <a class="btn btn-xs btn-success" href="{{ route('admin.bodAppliedJobs.candidatesViewJobStatus', [encrypt_data($candidate->id), 'path' => 'admin/bodAppliedJobs/' . $candidate->job_id]) }}">
