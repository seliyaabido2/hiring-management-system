...
<div class="card">
    <div class="card-header">
        {{ trans('cruds.bodSavedCandidates.title_singular') }} {{ trans('global.list') }}
    </div>
        ...
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-Role datatable-User">
                <thead>
                    <tr>
                        <th>
                            {{ trans('cruds.bodCandidates.fields.id') }}
                        </th>
                        <th>
                ...
                <tbody>
                    @foreach($bodCandidates as $key => $candidate)
                        <tr data-entry-id="{{ $candidate->id }}">
                            ...
                            <td>
                                <a href="{{ asset('candidate_resume').'/'.$candidate->resume}}" download>
                                    Download
                                </a>
                            ...
                            <td>
                                {{ $candidate->updated_at ? date('j-M-Y H:i:s', strtotime($candidate->updated_at)) : '' }}
                            ...
                            <td>
                                ...
                                @elseif ($candidate->status == 'Deactive')
                                    <button class="btn btn-xs btn-danger disabled">{{ trans('global.deactive') }}</button>
                                @endif
                            </td>
                            <td>
                                @can('candidate_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.bodCandidate.show', encrypt_data($candidate->id)) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('candidate_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.bodCandidate.edit', encrypt_data($candidate->id)) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                ...
                                @can('candidate_delete')
                                    ...
                                    <button class="btn btn-xs btn-danger delete-btn" data-id="{{$candidate->candidate_id}}">
                                        {{ trans('global.delete') }}
                                    </button>
                                @endcan
                                <a class="btn btn-xs btn-warning mt-1" href="{{ route('admin.bodCandidate.unSavedCandidate', ['id' =>encrypt_data($candidate->id) ]) }}">
                                  un-save
                                </a>
